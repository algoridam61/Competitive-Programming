...
void solve() {
    ...
    vvl onPath(9, vl(9, 1));
    // you have already visited that cell, we mark all the outside
    // padded cells as visited
    ...
    function<ll(ll, ll, ll)> count_paths = [&](ll i, ll x, ll y) -> ll {
        ...
        if (s[i] == '?') { 
            // 4 dirs 
            if (!onPath[x-1][y]) {
                ans += count_paths(i + 1, x - 1, y);
            }
            if (!onPath[x+1][y]) {
                ans += count_paths(i + 1, x + 1, y);
            }
            if (!onPath[x][y-1]) {
                ans += count_paths(i + 1, x, y - 1);
            }
            if (!onPath[x][y+1]) {
                ans += count_paths(i + 1, x, y + 1);
            }
