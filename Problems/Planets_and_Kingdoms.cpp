...
public:
    SCC(int n) : n(n), graph(n + 1), rev_graph(n + 1), component(n + 1, -1), visited(n + 1, false), comp_count(0) {}
    ...
    void find_components() {
        // Step 1: Run DFS on original graph and fill order
        for (int i = 1; i <= n; i++) {
            if (!visited[i]) {
                dfs1(i);
            }
        ...
        // Step 2: Run DFS on reverse graph in reverse order
    ...
    int get_component_count() { return comp_count; }
    int get_component(int v) { return component[v]; }
};
...
void solve() {
    ...
    cout << scc.get_component_count() << "\n";
    for (int i = 1; i <= n; i++) {
        cout << scc.get_component(i) << " ";
    }
    cout << "\n";
}
