...
void solve() {
    ...
    if (x % y == 0|| y%x == 0) {
        ...
        if (x % y == 0) {
            // if (z >=y && z <= x) {
                // closest number to z which is divisible by x and multiple of y
                vl divisors;
                ll num = x / y;
                for (ll i = 1; i*i <= num; i++) {
                    if (num % i == 0) {
                        divisors.push_back(i * y);
                        if (i != num / i) {
                            divisors.push_back((num / i) * y);
                        }
                    }
                }
                sortall(divisors);
                for (ll i = 0; i < divisors.size(); i++) {
                    ans = min(ans, c + abs(divisors[i] - z));
                }
                cout<<ans;L;return;
            // } else 
            // {
            ...
            // if (z >=x && z <= y) {
                ...
                for (ll i = 1; i*i <= num; i++) {
                    if (num % i == 0) {
                        divisors.push_back(i * x);
                        if (i != num / i) {
                            divisors.push_back((num / i) * x);
                        }
                    }
                }
                sortall(divisors);
