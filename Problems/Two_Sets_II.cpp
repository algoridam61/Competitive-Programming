...
*/
vvl dp;
ll solve_dp(ll curr_ind, ll target)
{
    if (target < 0 || curr_ind < 0) return 0;
    if (target == 0) return 1;
    if (dp[curr_ind][target] != -1) return dp[curr_ind][target];

    ll ans = solve_dp(curr_ind - 1, target) + solve_dp(curr_ind - 1, target - curr_ind);
    ans = ans % MOD;
    return dp[curr_ind][target] = ans;
}
void solve()
{
    ll n;cin>>n;
    ...
    else 
    {
        ...
        dp.resize(n+1, vl(target+1, -1));

        cout<<solve_dp(n-1, target);L;
        return;
