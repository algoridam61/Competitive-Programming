...
void solve() {
    ...
    auto sum_1_to_k = [&](ll k) {
        if (k < 0) return 0LL;
        ll k_mod = k % MOD;
        ll term1 = k_mod;
        ll term2 = (k_mod + 1) % MOD;
        ll res = (term1 * term2) % MOD;
        res = (res * inv2) % MOD;
        // this is actually division by 2 in modular arithmetic
        return res;
    };
    ...
    while (i <= n) {
        ...
        // Sum of indices in the current block [i, next_i - 1]
        ll sum_indices = (sum_1_to_k(next_i - 1) - sum_1_to_k(i - 1) + MOD) % MOD;
        
        // Contribution of this block: val * sum_indices
        ll term = ((val % MOD) * sum_indices) % MOD;
        sum = (sum + term) % MOD;
