...
void solve() {
    ...
    // similar to sieve algorithm
    // we need to mark the multiples of each prime number
    ...
    //     // cout<<curr_prime<<" ";
    //     if (curr_prime <= n) {
    //         is_prime[curr_prime] = true; // mark the prime number
    //     }
    // }

    // // out(is_prime);return;

    // // here list of available primes in a
    // vector<ll> primes;
    // for (ll i = 2; i <= n; ++i) {
    //     if (is_prime[i]) {
    //         primes.push_back(i);
    ...
    f(mask, 1<<k) {
        ...
        f(i,k) {
            if (mask & (1 << i)) {
                count++;
                if (n / a[i] < product) {
                    product = n + 1; // if product exceeds n, it will not contribute to multiples
        ...
        // cout<<mask<<" "<<count<<" "<<product<<"\n";
        if (product == 1) continue; // skip if product is 1, as it does not contribute to multiples
        if (count%2 == 0) {
            ans -= n / product; // even count, subtract
        } else {
            ans += n / product; // odd count, add
        }
    }
