...
void solve()
{
    ...
    ll ans = (((sum * sum) % MOD - sumsq + MOD) % MOD * ((MOD + 1) / 2)) % MOD;
    ans = (ans+MOD)%MOD;
    ll den = n*(n-1)/2;

    // output pq-1(mod(10^9+7))
    ll inv_den = 1, base = den, exp = MOD - 2;
    while (exp > 0) {
        if (exp % 2 == 1) {
            inv_den = (static_cast<__int128>(inv_den) * base) % MOD;
        }
        base = (static_cast<__int128>(base) * base) % MOD;
        exp /= 2;
    }

    ll result = (ans * inv_den) % MOD;
    cout << result;L;
