...
void solve() {
    ...
    vl start_timer(n+1), end_timer(n+1);
    ll timer = 0;
    function<void(ll, ll)> dfs = [&](ll node, ll parent) {
        start_timer[node] = ++timer;
        for (ll neighbor : graph[node]) {
            if (neighbor != parent) {
                dfs(neighbor, node);
            }
        }
        end_timer[node] = timer;
    };
    dfs(1, -1);
    ...
    for (int i = 1; i <= n; i++) {
        bit.add(start_timer[i]-1, values[i]);
        bit.add(end_timer[i], -values[i]);
    }   

    while (q--) {
        ll type; cin>>type;
        if (type == 1) {
            ll node, new_value; cin>>node>>new_value;
            bit.add(start_timer[node]-1, new_value - values[node]);
            bit.add(end_timer[node], -new_value + values[node]);
            values[node] = new_value;
        } else {
            ll node; cin>>node;
            cout << bit.pref_sum(start_timer[node]-1) << "\n";
        }
    }
