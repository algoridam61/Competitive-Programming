...
void solve() {
    ...
    f(i, n+1) {
        for (auto edge : edges) {
            ...
            if (dist[u] + w < dist[v]) {
                ...
                if (i == n) {
                    node = v;
                    neg_cycle = true; // If we can still relax on the nth iteration, there's a negative cycle
                }
            }
        }
    }
    ...
    if (!neg_cycle) {
        ...
        // fidn the cycle starting from the node
        for (ll i = 0; i < n; ++i) {
            node = parent[node];
        }
        // that node may actually be reachable from teh negative cycle that is why, distance got updated
        ll start = node;
