...
void solve() {
    ...
    // for each node, maintain k best distances from node 1
    vector<priority_queue<ll>> best_distances(n + 1);
    best_distances[1].push(0);
    ...
    while (!pq.empty()) {
        ...
        pq.pop();
        if (processed[node] >= k) {
            continue; // If we have already processed this node k times, skip it
        }
        processed[node]++;

        if (best_distances[node].size() > k) {
            continue;
        }

        for (auto neigh : graph[node]) {
            ll next_node = neigh.first;
            ll weight = neigh.second;
            ll new_dist = dist + weight;

            if (best_distances[next_node].size() < k || new_dist < best_distances[next_node].top()) {
                if (best_distances[next_node].size() == k) {
                    best_distances[next_node].pop();
                }
                best_distances[next_node].push(new_dist);
                pq.push({new_dist, next_node});
            }
        }
    }

    priority_queue<ll> result = best_distances[n];
    vl output;
    while (result.size()) {
