...
void solve()
{
    int n;cin>>n;
    int d,k;cin>>d>>k;
    vi l(n),r(n);multiset<int> left;multiset<int> right;
    f(i,k)
    {
        cin>>l[i]>>r[i];
        left.insert(l[i]);
    ...
    // we want duration to start from 1 and till n - d + 1
    ...
    f1(i,2,n-d+2)
    {
        // those having end as i - 1 are removed
        // those having start as d + i - 1 are included
        int remo = right.count(i-1);
        int adde = left.count(d+i-1);

        currd = currd - remo + adde;

        if (currd > currmax)
        {
            currmax = currd; currmaxi = i;
        }
        if (currd < currmin)
        {
            currmin = currd; currmini = i;
        }
    }

    cout<<currmaxi<<" "<<currmini;L;
