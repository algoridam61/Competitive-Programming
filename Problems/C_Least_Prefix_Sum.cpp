...
void solve()
{
    ...
    vl pref(n);
    pref[0] = a[0];
    f1(i,1,n) pref[i] = pref[i-1] + a[i];

    ll curr = pref[m-1];

    int i = m-2;
    int ans = 0;
    priority_queue<ll> pq; // maintain a max pq;
    pq.push(a[m-1]);
    while (i>=0)
    {
        if (pref[i]<curr)
        {
            ...
            while (pref[i]<curr)
            {
                // only elements from i+1 to m-1 will affect their difference
                curr-=2*pq.top(); // we deal with the biggest elements and reduce curr
                pq.pop(); 
                ans++;
            }
        }
    ...
    // cout<<ans;L;return;
    // ll reduced = pref[m-1] - curr;
    i = m;
    curr = pref[m-1];
    priority_queue<ll, vector<ll>, greater<ll>> pq1; // maintain a min queue;
    while (i<n)
    {
        pq1.push(a[i]);
        if (pref[i]<curr)
        {
            // we want to increase the element from m to i
            while (pref[i]<curr)
            {
                curr+=2*pq1.top();
                pq1.pop();
