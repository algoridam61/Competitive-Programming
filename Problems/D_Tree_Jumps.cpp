...
void solve() {
    ...
    while (!q.empty()) {
        ...
        for (ll child : graph[node]) {
            ...
            levels[level[child]].push_back(child);
            q.push(child);
        }
    }
    ...
    total[1] = 1;  // total[level] = sum of dp[all nodes at that level]
    dp[1] = 1;

    for (ll lev = 2; lev <= n; lev++) {
        // total[lev] = 0;
        for (ll node : levels[lev]) {
            // dp[node] = sum of total[all levels that can reach this node]
            // A node at level lev can be reached from levels 1 to lev-1
            if (lev == 2) dp[node] = total[lev - 1];
            else dp[node] = total[lev - 1] - dp[parent[node]];
            total[lev] = (total[lev] + dp[node] + MOD)%MOD;
        }
    }
