...
void solve()
{
    ...
    f(i, m)
    {
        ll u,v;cin>>u>>v;
        u--;v--;
        graph[v].push_back(u); // come from graph 
    }
    ...
    f1(i, 2, 1<<n)
    {
        ...
        f(new_end,n)
        {
            if (i & (1<<new_end) == 0) continue; // means this city is not in subset i

            ll prev = 1<<new_end ^ i;

            for (auto j : graph[new_end])
            {
                if (i & 1<<j) 
                {
                    dp[i][new_end] += dp[prev][j];
                    dp[i][new_end] %= MOD;
                }
            }
        }
