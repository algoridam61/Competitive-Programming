...
int main() {
    ...
    dp[0] = 1;
    for (ll i = 0; i < palindromes.size() && palindromes[i] <= 40000; i++) {
        for (ll j = palindromes[i]; j <= 40000; j++) {
            dp[j] = (dp[j] + dp[j - palindromes[i]]) % MOD;
        }
    }
    ...
    // number of ways of expressing n as a sum of palindromes
