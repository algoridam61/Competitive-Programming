...
void solve() {
    ...
    f1(i, 2, 20) {
        index_start[i] = index_start[i-1] + num_len * num* 9;
        num_len ++;
        num *= 10;
    }
    ...
    while (q--) {
        ...
        ll diff = (k - index_start[len])/len;
        ll number = num_lower + diff;
        ll ind = (k - index_start[len]) % len;

        if (ind < 0) {
            ind += len;
        }
        // cout<<number<<" "<<ind<<" "<<len<<endl;
