...
void solve() {
    ...
    f1(i, 1, n+1) {
        f1(j, x[i] - r[i], x[i] + r[i] + 1) {
            if (point_on_x.find(j) == point_on_x.end()) {
                point_on_x.insert(j);
                ans[j] = r[i] * r[i] - (j - x[i]) * (j - x[i]);
            } else {
                ans[j] = max(ans[j], r[i] * r[i] - (j - x[i]) * (j - x[i]));
            }
        }
    }
    ll asn = 0;
    for (auto & p : point_on_x) {
        asn += 2*(ll)(floor(sqrt(double(ans[p])))) + 1;
    }
    cout<<asn<<"\n";
}
/*
