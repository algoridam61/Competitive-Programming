...
void solve()
{
    ...
    // cout<<"kevins: "<<kevrating;L;
    vl part;
    f(i,n)
    {
        if (a[i]>=kevrating) part.push_back(a[i]);
    }
    sortall(part);
    ...
    f(i,m)
    {
        // number of elements greater than b[i] 
        // find the smallest index in a which is greater than equal to b[i].
        ll start = 0;ll end = size(part)-1;
        ll mid;
        ll ans=-1;
        while (start<=end)
        {
        ...
        if (ans==-1) c[i]=0;
        else c[i]=part.size()-ans;
    }
    f(i,m)
    {
        if (c[i]==part.size()) c[i]=0;
    }
    sortall(c);
