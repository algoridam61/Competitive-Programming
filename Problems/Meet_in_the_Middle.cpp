...
//     unordered_map<ll, ll> sum_count1, sum_count2;
//     sum_count1.reserve(1LL << m1);
//     sum_count2.reserve(1LL << m2);
//     for (ll mask = 0; mask < (1LL << m1); ++mask) {
//         ll sum = 0;
//         for (ll i = 0; i < m1; ++i) {
//             if (mask & (1LL << i)) {
//                 sum += arr1[i];
...
//     for (ll mask = 0; mask < (1LL << m2); ++mask) {
//         ll sum = 0;
//         for (ll i = 0; i < m2; ++i) {
//             if (mask & (1LL << i)) {
//                 sum += arr2[i];
...
//     ll total_count = 0;
//     for (const auto& [sum1, count1] : sum_count1) {
//         ll num1 = count1;
//         ll target = k - sum1;
//         total_count += num1 * sum_count2[target];
//     }
...
void solve() {
    ...
    for (ll mask = 0; mask < (1LL << mid); ++mask) {
        ll sum = 0;
        for (ll i = 0; i < mid; ++i)
    ...
    for (ll mask = 0; mask < (1LL << (n - mid)); ++mask) {
        ll sum = 0;
        for (ll i = 0; i < n - mid; ++i)
            if (mask & (1LL << i)) sum += a[mid + i];
    ...
    for (ll s : left_sums) {
        ll rem = x - s;
        ans += upper_bound(right_sums.begin(), right_sums.end(), rem)
             - lower_bound(right_sums.begin(), right_sums.end(), rem);
    }
