...
void solve() {
    ...
    // all of these are hashing problems
    ll n = s.size();
    ...
    vl pref(n+1);
    f1(i, 1, n+1) {
        pref[i] = (pref[i - 1]*base + s[i-1]-'a') % MOD;
    }

    auto get_hash = [&](ll l, ll r) {
        ll hash_value = (pref[r + 1] - (pref[l] * base_powers[r - l + 1]) % MOD + MOD) % MOD;
        return hash_value;
    };
    ...
    f(i,n) {
        ...
        bool poss = true;
        while (curr < n) {
            ll length = min(n - curr, (ll)(i + 1));
            ll hash1 = get_hash(0, 0 + length - 1);
            ll hash2 = get_hash(curr, curr + length - 1);
            if (hash1 != hash2) {
                poss = false;
                break;
            }
